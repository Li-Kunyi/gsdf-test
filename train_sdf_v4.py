...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, save_ckpt, ckpt_pth, debug_from):
    ...
    for iteration in range(first_iter, opt.iterations + 1):        
        ...
        if torch.isnan(distortion_loss):
            print("distortion_loss NAN")
        if torch.isnan(rgb_loss):
            print("rgb_loss NAN")
        if torch.isnan(depth_normal_loss):
            print("depth_normal_loss NAN")
        ...
        if torch.isnan(gaussian_sdf_loss):
            print("gaussian_sdf_loss NAN")
        ...
        if opt.lambda_gradient_consistency > 0:
            ...
            if torch.isnan(consistency_loss):
                print("consistency_loss NAN")

        with torch.no_grad():
            tb_writer.add_scalar('train_loss_patches/gaussian_sdf_loss', gaussian_sdf_loss.item(), iteration)
        ...
        if iteration < opt.iterations:
            ...
            if iteration > opt.start_train_sdf:
                ...
                for inner_iter in range(n_inner_iter):
                    ...
                    n_sample_surface = 11 if iteration < opt.densify_until_iter else opt.n_sample_surface
                    fs_loss, sdf_loss = get_sdf_loss_with_gaussian_depth(gaussians, c2w, fx, fy, 
                                                                                    depth, render_alpha, render_normal_world.permute(1, 2, 0),
                    ...
                    lambda_fs = 0.0 if iteration < opt.densify_until_iter else opt.lambda_fs
                    grid_loss = lambda_sdf * sdf_loss + lambda_fs * fs_loss
                    if torch.isnan(fs_loss):
                        print("fs_loss NAN")
                    if torch.isnan(sdf_loss):
                        print("sdf_loss NAN")

                    if opt.lambda_smooth > 0 and iteration > opt.densify_until_iter:
                        smooth_loss = opt.lambda_smooth * smoothness(gaussians, sample_points=opt.smooth_sample_point, voxel_size=opt.smooth_voxel_size)
                        grid_loss += smooth_loss
                        if torch.isnan(smooth_loss):
                            print("smooth_loss NAN")
